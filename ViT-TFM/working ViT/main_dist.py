...
def main():
    ...
    # Preparing dataset
    dspl = h5py.File('../data/Test data/test_displacements_50.h5')["data"]
    trac = h5py.File('../data/Test data/test_sep_tractions_50.h5')["data"]

    if args.use_multi_task:
        dspl = np.moveaxis(np.array(dspl[:, :, :, :]), 3, 1)
        trac_separated = np.moveaxis(np.array(trac[:, :, :, :]), 4, 2)

        dspl_train = dspl[0:45, :, :, :]
        dspl_val = dspl[45:, :, :, :]
        trac_train = trac_separated[0:45, :, :, :, :]
        trac_val = trac_separated[45:, :, :, :, :]
    else:
        dspl = np.moveaxis(np.array(dspl), 3, 1)
    ...
    if args.continue_training:
        checkpoint = torch.load(
            '/ViT-TFM/working ViT/logs_and_weights/ViT-2022-Dec-10 11:33:45/ViT-2022-Dec-10 11:33:45_best_val_loss_2.649155e-06.pth')
