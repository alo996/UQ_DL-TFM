...
# Implementation of Vision Transformer modules. Largely based on https://pypi.org/project/timm/0.3.3/.
...
class RecTracHeadProb(nn.Module):
    def __init__(self, in_dim, patch_size=8):
        super().__init__()
...
class VisionTransformerProb(nn.Module):
    """
    Implementation of Prob-ViT.
    Parameters
    __________
    dspl_size : int
        Height and width of square displacement field.
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(embed_dim, eps=1e-7)
        self.rec_trac_head = RecTracHeadProb(embed_dim, patch_size)
    ...
    def forward(self, x, return_attention=False):
        ...
        _______
        logits : torch.Tensor
            Predicted traction fields, shape `(n_samples, 4, dspl_size, dspl_size)`
        """
        x = self.patch_embed(x)
        x = x + self.pos_embed  # (n_samples, n_patches, embed_dim)
        x = self.pos_drop(x)
